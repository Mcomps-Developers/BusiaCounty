<div class="ftAreaWrap position-relative bg-gDark fontAlter">
    <aside class="pt-4 pb-3 text-center ftConnectAside pt-md-7 pb-md-7 text-md-left">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-12 col-lg-7">
                    <nav class="mb-4 ftcaNav mb-lg-0">
                        <ul class="flex-wrap mb-0 list-unstyled d-flex justify-content-center justify-content-lg-start">
                            <li>
                                <a href="{{ route('about') }}">About</a>
                            </li>
                            <li>
                                <a href="{{ route('subcounties') }}">Sub-Counties</a>
                            </li>
                            <li>
                                <a href="{{ route('events') }}">Events</a>
                            </li>
                            <li>
                                <a href="{{ route('news.speeches') }}">Media</a>
                            </li>
                            <li>
                                <a href="{{ route('contact') }}">Contact</a>
                            </li>

                        </ul>
                    </nav>
                </div>
                <div class="col-12 col-lg-5">
                    <div
                        class="ctConnectWrap d-sm-flex justify-content-sm-center justify-content-lg-end align-items-sm-center">
                        <strong class="flex-shrink-0 mb-1 title font-weight-normal mr-sm-3 d-block">Connect With
                            Us</strong>
                        <ul
                            class="flex-wrap mb-0 list-unstyled socialNetworks ftSocialNetworks d-flex justify-content-center justify-content-sm-end">
                            <li>
                                <a href="javascript:void(0);">
                                    <i class="fab fa-facebook-square"><span class="sr-only">facebook</span></i>
                                </a>
                            </li>
                            <li>
                                <a href="javascript:void(0);">
                                    <i class="fab fa-twitter"><span class="sr-only">twitter</span></i>
                                </a>
                            </li>
                            <li>
                                <a href="javascript:void(0);">
                                    <i class="fab fa-instagram"><span class="sr-only">instagram</span></i>
                                </a>
                            </li>
                            <li>
                                <a href="javascript:void(0);">
                                    <i class="fab fa-youtube"><span class="sr-only">youtube</span></i>
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </aside>
    <!-- footerAside -->
    <aside class="footerAside pt-9 pb-sm-2 pt-xl-14 pb-xl-12">
        <div class="container">
            <div class="row">
                <div class="mb-6 col-12 col-sm-6 col-md-5 col-xl-3">
                    <div class="mb-6 ftLogo mt-md-1">
                        <a href="/">
                            <img src="{{ asset('assets/images/logoWhite.png') }}" class="img-fluid" alt="egovt">
                        </a>
                    </div>
                    <address class="mb-0 ftPlace">
                        <p class="mb-2"><strong class="font-weight-normal">Busia, Kenya</strong></p>
                        <ul class="mb-0 list-unstyled ftpScheduleList">
                            <li>
                                <i class="mr-1 fa fa-clock fwSemiBold mr-sm-0"></i>
                                <strong class="text-white title font-weight-normal">Opening Hours:</strong>
                                <br>
                                <time datetime="2011-01-12">Mon – Fri: 8:00 am – 5:00 pm</time>
                            </li>
                            <li>
                                <i class="mr-1 fa fa-phone-alt mr-sm-0"></i>
                                <strong class="text-white title font-weight-normal">Phone:</strong>
                                <a href="tel:+254700000000">+254700000000</a>
                            </li>
                            <li>
                                <i class="mr-1 fa fa-envelope mr-sm-0"></i>
                                <strong class="text-white title font-weight-normal">Email:</strong>
                                <a href="mailto:info@busiacounty.go.ke">info@busiacounty.go.ke</a>
                            </li>
                        </ul>
                    </address>
                </div>
                <div class="mb-6 col-12 col-sm-6 col-md-4 col-xl-3 col-xlwd-2">
                    <h3 class="mb-4 text-white ftHeading">Quick Links</h3>
                    <ul class="mb-0 list-unstyled ftsrLinksList">
                        <li>
                            <a href="{{ route('departments') }}">Departments</a>
                        </li>
                        <li>
                            <a href="{{ route('governor') }}">Governorship</a>
                        </li>
                        <li>
                            <a href="{{ route('executive') }}">The Executive</a>
                        </li>
                        <li>
                            <a href="{{ route('public.service') }}">Public Service</a>
                        </li>

                    </ul>
                </div>
                <div class="mb-6 col-12 col-sm-4 col-md-3 col-xl-2 col-xlwd-3">
                    <div class="pl-xlwd-11">
                        <h3 class="mb-4 text-white ftHeading">External Links</h3>
                        <ul class="mb-0 list-unstyled ftsrLinksList">
                            <li>
                                <a href="https://bajetiyetu.treasury.go.ke/site/county?ctid=40" target="_/blank">Bajeti
                                    Yetu</a>
                            </li>
                            <li>
                                <a href="javascript:void(0);">Council of Governors</a>
                            </li>
                            <li>
                                <a href="javascript:void(0);">E-Citizen</a>
                            </li>
                            <li>
                                <a href="javascript:void(0);">Public Service</a>
                            </li>
                            <li>
                                <a href="javascript:void(0);">Judiciary</a>
                            </li>
                            <li>
                                <a href="javascript:void(0);">National Police Service</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="mb-6 col-12 col-sm-8 col-md-12 col-xl-4">
                    <div class="ml-xl-n1 ml-xlwd-n7">
                        <h3 class="mb-5 text-white ftHeading">Mailing List</h3>
                        <form action="#" class="ftSubscribeForm">
                            <label class="d-block mb-7">Be the first to get the latest news, articles, and resources,
                                sent
                                straight to your email.</label>
                            <div class="mb-3 input-group">
                                <input type="text" class="form-control form-control-lg"
                                    placeholder="Enter Your Email">
                                <div class="input-group-append">
                                    <button type="button"
                                        class="p-0 border-0 btn btnTheme d-flex font-weight-bold text-capitalize position-relative"
                                        data-hover="Send">
                                        <span class="d-block btnText">Subscribe</span>
                                    </button>
                                </div>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </aside>
    <!-- pageFooter -->
    <footer id="pageFooter" class="pt-6 pb-3 text-center bg-dark pt-md-8 pb-md-5">
        <div class="container">
            <p>&copy; {{ date('Y') }} <a href="javascript:void(0);">{{ config('app.name') }}</a>. All Rights
                Reserved.
                Powered by <a target="_/blank" href="https://mcomps.co.ke" style="color: white"><b>Mcomps</b></a>. <br
                    class="d-md-none"></p>
        </div>
    </footer>
</div>
